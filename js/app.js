async function fetchJSONData() {
    ...
    video_player.addEventListener("timeupdate", function() {
        ...
        if (this.currentTime >= quiz[0]["quiz"][x]["clip_end"]) {
            ...
            if (quiz[0]["quiz"][x]["true"]) {
                truefalse_area.style.display = "flex";
                truefalse_title.innerHTML = quiz[0]["quiz"][x]["question"];
            } else {
...
function main() {
    ...
    let submitTrueFalseBtn = document.getElementById("submit-tfanswer");
    submitTrueFalseBtn.addEventListener("click", processAnswer);
